...
function drawSurface() {
    ...
    for (let j = 0; j <= POINTS; j++) {
        // const v = j * (-0.95 * Math.PI) / POINTS + 0.95 * Math.PI;
        const u = j * Math.PI / POINTS - Math.PI / 2
        for (let i = 0; i <= POINTS; i++) {
            // const v = i * Math.PI / POINTS;
            const v = i * (0.985 * Math.PI) / POINTS
            const fi = (-u / Math.sqrt(C + 1)) + Math.atan(Math.sqrt(C + 1) * Math.tan(u))
            const a = 2 / (C + 1 - C * Math.pow(Math.sin(v), 2) * Math.pow(Math.cos(u), 2))
            const r = a / Math.sqrt(C) * Math.sqrt((C + 1) * (1 + C * Math.pow(Math.sin(u), 2))) * Math.sin(v)
    ...
    gl.uniform4fv(iColor, [0.4, 0.4, 0.7, 1]);
    gl.uniform1f(iColorCoef, 0.0);
...
function draw() {
    ...
    const matAccum1 = m4.multiply(translateToPointZero, getRotationMatrix(orientation.alpha, orientation.beta, orientation.gamma));
